...
static HQResponseType zx_responseType = kHQResponseTypeJSON;
static HQRequestType zx_requestType = kHQRequestTypeJSON;
static HQNetworkStatus zx_networkStatus = kHQNetworkStatusUnknown;
...
+ (unsigned long long)totalCacheSize
     ...
     */
+ (void)comfigRequestType:(HQRequestType)requestType responseType:(HQResponseType)responseType shouldAutoEncodeUrl:(BOOL)shouldAutoEncode callbackOnCancelRequsest:(BOOL) shouldCallbackOnCancelRequest
    {
...
+ (HQURLSessionTask *)getWithUrl:(NSString *)url success:(HQResponseSuccess)success fail:(HQResponseError)fail
    {
     ...
     */
+ (HQURLSessionTask *)postWithUrl:(NSString *)url params:(NSDictionary *)params success:(HQResponseSuccess)success fail:(HQResponseError)fail
    {
     ...
     *  @param fail               请求失败
     */
+ (void)updataVideoWithUrl:(NSString *)url withParams:(NSDictionary *)params withImage:(NSArray *)arrImage withCompressionQuality:(CGFloat)compressionQuality success:(HQResponseSuccess)success fail:(HQResponseError)fail
    {
        //检查参数是否正确
        if (!url|| !arrImage) {
            NSLog(@"参数不完整");
     ...
     */
+ (HQURLSessionTask *)WZX_requestWithUrl:(NSString *)url refreshCache:(BOOL)refreshCache httpMedth:(NSUInteger)httpMethod params:(NSDictionary *)params success:(HQResponseSuccess)success fail:(HQResponseError)fail{
    ...
    if(httpMethod == 1)
    {
        if(zx_cacheGet)
        {
            ...
            {
                if(zx_networkStatus == kHQNetworkStatusNotReachable || zx_networkStatus == kHQNetworkStatusUnknown)
                {
                    id response = [HQNetworking cahceResponseWithURL:absolute parameters:params];
        ...
        session = [manager GET:url parameters:params progress:^(NSProgress * _Nonnull downloadProgress) {
            ...
            if ([error code] < 0 && zx_cacheGet) {// 获取缓存
                id response = [HQNetworking cahceResponseWithURL:absolute
                                                       parameters:params];
                if (response) {
                    if (success) {
                        [self successResponse:response callback:success];
...
+ (void)detectNetwork
    {
        ...
        [reachabilityManager setReachabilityStatusChangeBlock:^(AFNetworkReachabilityStatus status) {
            ...
            {
                zx_networkStatus = kHQNetworkStatusNotReachable;
            }else if (status == AFNetworkReachabilityStatusUnknown)
            {
                zx_networkStatus = kHQNetworkStatusUnknown;
            }else if (status == AFNetworkReachabilityStatusReachableViaWWAN)
            {
                zx_networkStatus = kHQNetworkStatusReachableViaWWAN;
            }else if (status == AFNetworkReachabilityStatusReachableViaWiFi)
            {
                zx_networkStatus = kHQNetworkStatusReachableViaWiFi;
            }
...
+ (void)successResponse:(id)responseData callback:(HQResponseSuccess)success
    {
        if(success)
        {
...
+ (void)handleCallbackWithError:(NSError *)error fail:(HQResponseError)fail
    {
        if([error code] == NSURLErrorCancelled)
        {
